/**
 * Star Wars Unlimited Set Image Generator using Higgsfield Cloud API
 *
 * This script generates banner images for Star Wars Unlimited TCG sets using the Higgsfield AI API.
 *
 * Usage:
 *   npx tsx scripts/generate-starwars-images.ts [--start <index>] [--count <number>] [--dry-run]
 *
...
import { STARWARS_SET_PROMPTS, TOTAL_PROMPTS } from './data/starwars-set-prompts'
...
const CONFIG = {
  ...
  outputDir: 'scripts/output/starwars-images',
  // Progress file to track completed generations
  progressFile: 'scripts/output/starwars-images-progress.json'
}
...
async function main() {
  ...
  console.log('\n\x1b[36m╔══════════════════════════════════════════════════════════╗\x1b[0m')
  console.log('\x1b[36m║   Star Wars Unlimited Set Image Generator - Higgsfield   ║\x1b[0m')
  ...
  const setsToProcess = STARWARS_SET_PROMPTS.slice(startIndex, startIndex + countArg).filter(
  ...
  if (dryRun) {
    ...
    setsToProcess.forEach((set, i) => {
      console.log(`\x1b[36m[${i + 1}] ${set.id} - ${set.name}\x1b[0m`)
      console.log(`    Type: ${set.type}`)
      console.log(`    Prompt: ${set.prompt.substring(0, 120)}...`)
      console.log('')
    })
    return
  }
