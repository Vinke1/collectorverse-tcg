/**
 * Script pour re-seeder les cartes OP06 EN One Piece
 * Source: https://www.opecards.fr/cards/search?page=1&sortBy=releaseR&serie=339&language=EN
 *
 * Ce script:
 * 1. Supprime toutes les images OP06/en existantes dans Supabase Storage
 * 2. Supprime toutes les cartes EN existantes de la serie OP06
 * 3. Scrape les 6 pages de OP06 EN sur opecards.fr
 ...
 *
 * Usage:
 *   npx tsx scripts/reseed-onepiece-op06-en.ts
 *   npx tsx scripts/reseed-onepiece-op06-en.ts --dry-run
 *   npx tsx scripts/reseed-onepiece-op06-en.ts --skip-images
 */
...
const OP06_SEARCH_URL = `${BASE_URL}/cards/search?sortBy=number&serie=339&language=EN`
const TOTAL_PAGES = 6
...
interface OP06Card {
  ...
  displayNumber: string // Format: "OP06-001"
  name: string
  rarity: string
...
/**
 * Parse une URL de carte OP06
 * Format: /cards/en-op06-{number}-{rarity}-{name}
 * Exemple: /cards/en-op06-001-l-uta
 */
function parseOP06Url(url: string): OP06Card | null {
  const slug = url.replace('/cards/', '')

  // Pattern: en-op06-{number}-{rarity}-{name}
  const match = slug.match(/^en-op06-(\d{3})-([a-z]+)-(.+)$/i)
...
/**
 * Scrape toutes les URLs de cartes OP06 EN depuis les 6 pages
 */
async function scrapeOP06CardUrls(browser: Browser): Promise<string[]> {
  ...
  try {
    for (let pageNum = 1; pageNum <= TOTAL_PAGES; pageNum++) {
      const pageUrl = `${OP06_SEARCH_URL}&page=${pageNum}`
      ...
      // Extraire les URLs de cartes OP06 EN
      const cardUrls = await page.evaluate(() => {
        const links = Array.from(document.querySelectorAll('a[href*="/cards/"]'))
          ...
          .filter(href => {
            ...
            // Accepter uniquement les cartes OP06 EN avec prefixe en-
            // Pattern: /cards/en-op06-{number}-{rarity}-...
            return /^\/cards\/en-op06-\d{3}-[a-z]+-/i.test(path)
          })
  ...
  logger.success(`Total: ${uniqueUrls.length} URLs de cartes OP06 uniques`)
 ...
 */
async function fetchCardDetails(page: Page, card: OP06Card): Promise<OP06Card> {
  try {
...
/**
 * Recupere l'ID de la serie OP06 pour One Piece
 */
async function getOP06SeriesId(): Promise<string> {
  ...
  if (seriesError || !series) {
    throw new Error('Serie OP06 non trouvee pour One Piece')
  }
...
/**
 * Supprime toutes les cartes EN de la serie OP06
 */
 ...
 */
async function insertCard(seriesId: string, card: OP06Card): Promise<boolean> {
  if (dryRun) {
...
async function main() {
  logger.section('Re-seeding One Piece OP06 EN')
  console.log(`Source: ${OP06_SEARCH_URL}`)
  console.log(`Mode: ${dryRun ? 'DRY-RUN' : 'PRODUCTION'}`)
  console.log(`Skip images: ${skipImages}`)

  try {
    // 1. Recuperer l'ID de la serie
    logger.info('\n1. Recuperation de la serie OP06...')
    const seriesId = await getOP06SeriesId()
    logger.success(`Serie OP06 trouvee: ${seriesId}`)

    // 2. Supprimer les images existantes
    if (!skipImages && !dryRun) {
      logger.section('Suppression des images existantes OP06/en')
      const deleteResult = await deleteOnePieceSeriesImages('OP06', 'en')
    ...
    try {
      // 4. Scraper les URLs
      logger.info('\n3. Scraping des URLs de cartes OP06 EN...')
      const cardUrls = await scrapeOP06CardUrls(browser)

      // 5. Parser les URLs
      logger.info('\n4. Parsing des URLs...')
      const cards: OP06Card[] = []

      for (const url of cardUrls) {
        const parsed = parseOP06Url(new URL(url).pathname)
        if (parsed) {
          cards.push(parsed)
        }
      }
    ...
    if (dryRun) {
      ...
      console.log('\nVerifiez: https://www.collectorverse.io/series/onepiece/OP06')
    }
