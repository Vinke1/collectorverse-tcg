/**
 * Script pour re-seeder les cartes OP10 One Piece (EN et FR)
 * Source FR: https://www.opecards.fr/cards/search?page=1&sortBy=releaseR&serie=498&language=FR (6 pages)
 * Source EN: https://www.opecards.fr/cards/search?page=1&sortBy=releaseR&serie=557&language=EN (6 pages)
 *
 * Ce script:
 * 1. Supprime toutes les images OP10 existantes dans Supabase Storage
 * 2. Supprime toutes les cartes existantes de la serie OP10
 * 3. Scrape les pages de OP10 sur opecards.fr (EN et FR)
 ...
 *
 * Usage:
 *   npx tsx scripts/reseed-onepiece-op10.ts
 *   npx tsx scripts/reseed-onepiece-op10.ts --dry-run
 *   npx tsx scripts/reseed-onepiece-op10.ts --lang=en
 *   npx tsx scripts/reseed-onepiece-op10.ts --lang=fr
 *   npx tsx scripts/reseed-onepiece-op10.ts --skip-images
 */
...
// Note: FR n'a pas de prefixe de langue dans les URLs (op10-xxx) contrairement a EN (en-op10-xxx)
const LANG_CONFIG = {
  en: {
    searchUrl: `${BASE_URL}/cards/search?sortBy=releaseR&serie=557&language=EN`,
    totalPages: 6,
    urlPrefix: 'en-op10',
    urlPattern: /^\/cards\/en-op10-(\d{3})-([a-z]+)-(.+)$/i,
    storageLang: 'en'
  },
  fr: {
    searchUrl: `${BASE_URL}/cards/search?sortBy=releaseR&serie=498&language=FR`,
    totalPages: 6,
    urlPrefix: 'op10', // FR n'a pas de prefixe de langue
    urlPattern: /^\/cards\/op10-(\d{3})-([a-z]+)-(.+)$/i,
    storageLang: 'fr'
  }
}
...
interface OP10Card {
  ...
  displayNumber: string // Format: "OP10-001"
  name: string
  rarity: string
...
/**
 * Parse une URL de carte OP10
 * Format EN: /cards/en-op10-{number}-{rarity}-{name}
 * Format FR: /cards/op10-{number}-{rarity}-{name} (pas de prefixe fr-)
 * Exemple: /cards/en-op10-001-l-shanks
 */
function parseOP10Url(url: string, lang: 'en' | 'fr'): OP10Card | null {
...
/**
 * Scrape toutes les URLs de cartes OP10 depuis les pages de recherche
 */
async function scrapeOP10CardUrls(browser: Browser, lang: 'en' | 'fr'): Promise<string[]> {
  const config = LANG_CONFIG[lang]
  ...
  const urlPrefix = config.urlPrefix // 'en-op10' pour EN, 'op10' pour FR
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      // Extraire les URLs de cartes OP10
      const cardUrls = await page.evaluate((prefix) => {
        const links = Array.from(document.querySelectorAll('a[href*="/cards/"]'))
          ...
          .filter(href => {
            ...
            // Accepter uniquement les cartes OP10 avec le bon prefixe
            // EN: /cards/en-op10-{number}-{rarity}-...
            // FR: /cards/op10-{number}-{rarity}-... (pas de prefixe fr-)
  ...
  logger.success(`Total ${lang.toUpperCase()}: ${uniqueUrls.length} URLs de cartes OP10 uniques`)
 ...
 */
async function fetchCardDetails(page: Page, card: OP10Card): Promise<OP10Card> {
  try {
...
/**
 * Recupere l'ID de la serie OP10 pour One Piece
 */
async function getOP10SeriesId(): Promise<string> {
  ...
  if (seriesError || !series) {
    throw new Error('Serie OP10 non trouvee pour One Piece')
  }
...
/**
 * Supprime toutes les cartes d'une langue de la serie OP10
 */
 ...
 */
async function insertCard(seriesId: string, card: OP10Card): Promise<boolean> {
  if (dryRun) {
...
async function processLanguage(browser: Browser, seriesId: string, lang: 'en' | 'fr'): Promise<{ success: number; error: number }> {
  ...
  if (!skipImages && !dryRun) {
    logger.info(`Suppression des images existantes OP10/${lang}`)
    const deleteResult = await deleteOnePieceSeriesImages('OP10', lang)
  ...
  // 2. Scraper les URLs
  logger.info(`Scraping des URLs de cartes OP10 ${langUpper}...`)
  const cardUrls = await scrapeOP10CardUrls(browser, lang)

  // 3. Parser les URLs
  logger.info('Parsing des URLs...')
  const cards: OP10Card[] = []

  for (const url of cardUrls) {
    const parsed = parseOP10Url(new URL(url).pathname, lang)
    if (parsed) {
      cards.push(parsed)
    }
  }
...
async function main() {
  logger.section('Re-seeding One Piece OP10')
  ...
  try {
    ...
    logger.info('\n1. Recuperation de la serie OP10...')
    const seriesId = await getOP10SeriesId()
    logger.success(`Serie OP10 trouvee: ${seriesId}`)
    ...
    if (dryRun) {
      ...
      console.log('\nVerifiez: https://www.collectorverse.io/series/onepiece/OP10')
    }
