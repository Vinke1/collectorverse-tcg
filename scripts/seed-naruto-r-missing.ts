/**
 * Script pour ajouter les cartes R-111 à R-210 manquantes de Naruto Kayou
 *
 * Sources:
 * - R-111 à R-160: https://narutopia.fr/wp-content/uploads/2024/09/NRZ06-R-{num}.webp
 * - R-161 à R-210: https://narutopia.fr/wp-content/uploads/2024/12/NRB07-R-{num}L1.webp
 *
 * Usage:
 *   npx tsx scripts/seed-naruto-r-missing.ts              # Exécuter
 *   npx tsx scripts/seed-naruto-r-missing.ts --dry-run    # Mode test
 */
...
interface CardToAdd {
  number: number
  sourceUrl: string
  targetNumber: string  // Format: R-111, R-112, etc.
}
...
function generateCardList(): CardToAdd[] {
  ...
  // R-111 à R-160 (NRZ06)
  for (let num = 111; num <= 160; num++) {
    cards.push({
      number: num,
      sourceUrl: `https://narutopia.fr/wp-content/uploads/2024/09/NRZ06-R-${num}.webp`,
      targetNumber: `R-${num.toString().padStart(3, '0')}`
    })
  }

  // R-161 à R-210 (NRB07 avec suffixe L1)
  for (let num = 161; num <= 210; num++) {
    cards.push({
      number: num,
      sourceUrl: `https://narutopia.fr/wp-content/uploads/2024/12/NRB07-R-${num}L1.webp`,
      targetNumber: `R-${num.toString().padStart(3, '0')}`
    })
  }
 ...
 */
async function main() {
  logger.section('Ajout des cartes R-111 à R-210 Naruto Kayou')
  ...
  try {
    ...
    logger.info(`${cards.length} cartes à traiter (R-111 à R-210)`)
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      const { error } = await supabase
        ...
        .upsert({
          ...
          language: LANGUAGE,
          rarity: NARUTO_RARITY_DB_MAPPING[RARITY_CODE] || 'rare',
