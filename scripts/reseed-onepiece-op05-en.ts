/**
 * Script pour re-seeder les cartes OP05 EN One Piece
 * Source: https://www.opecards.fr/cards/search?page=1&sortBy=releaseR&serie=300&language=EN
 *
 * Ce script:
 * 1. Supprime toutes les images OP05/en existantes dans Supabase Storage
 * 2. Supprime toutes les cartes EN existantes de la serie OP05
 * 3. Scrape les 6 pages de OP05 EN sur opecards.fr
 * 4. Telecharge les images et les uploade sur Supabase Storage
 * 5. Insere les nouvelles cartes avec le bon numbering (sans /122) et noms corrects
 *
 * Usage:
 *   npx tsx scripts/reseed-onepiece-op05-en.ts
 *   npx tsx scripts/reseed-onepiece-op05-en.ts --dry-run
 *   npx tsx scripts/reseed-onepiece-op05-en.ts --skip-images
 */
...
const OP05_SEARCH_URL = `${BASE_URL}/cards/search?sortBy=number&serie=300&language=EN`
const TOTAL_PAGES = 6
...
interface OP05Card {
  ...
  displayNumber: string // Format: "OP05-001"
  name: string
  rarity: string
...
/**
 * Parse une URL de carte OP05
 * Format: /cards/en-op05-{number}-{rarity}-{name}
 * Exemple: /cards/en-op05-001-l-trafalgarlaw
 */
function parseOP05Url(url: string): OP05Card | null {
  const slug = url.replace('/cards/', '')

  // Pattern: en-op05-{number}-{rarity}-{name}
  const match = slug.match(/^en-op05-(\d{3})-([a-z]+)-(.+)$/i)
...
/**
 * Scrape toutes les URLs de cartes OP05 EN depuis les 6 pages
 */
async function scrapeOP05CardUrls(browser: Browser): Promise<string[]> {
  ...
  try {
    for (let pageNum = 1; pageNum <= TOTAL_PAGES; pageNum++) {
      const pageUrl = `${OP05_SEARCH_URL}&page=${pageNum}`
      ...
      // Extraire les URLs de cartes OP05 EN
      const cardUrls = await page.evaluate(() => {
        const links = Array.from(document.querySelectorAll('a[href*="/cards/"]'))
          ...
          .filter(href => {
            ...
            // Accepter uniquement les cartes OP05 EN avec prefixe en-
            // Pattern: /cards/en-op05-{number}-{rarity}-...
            return /^\/cards\/en-op05-\d{3}-[a-z]+-/i.test(path)
          })
  ...
  logger.success(`Total: ${uniqueUrls.length} URLs de cartes OP05 uniques`)
 ...
 */
async function fetchCardDetails(page: Page, card: OP05Card): Promise<OP05Card> {
  try {
...
/**
 * Recupere l'ID de la serie OP05 pour One Piece
 */
async function getOP05SeriesId(): Promise<string> {
  ...
  if (seriesError || !series) {
    throw new Error('Serie OP05 non trouvee pour One Piece')
  }
...
/**
 * Supprime toutes les cartes EN de la serie OP05
 */
 ...
 */
async function insertCard(seriesId: string, card: OP05Card): Promise<boolean> {
  if (dryRun) {
...
async function main() {
  logger.section('Re-seeding One Piece OP05 EN')
  console.log(`Source: ${OP05_SEARCH_URL}`)
  console.log(`Mode: ${dryRun ? 'DRY-RUN' : 'PRODUCTION'}`)
  console.log(`Skip images: ${skipImages}`)

  try {
    // 1. Recuperer l'ID de la serie
    logger.info('\n1. Recuperation de la serie OP05...')
    const seriesId = await getOP05SeriesId()
    logger.success(`Serie OP05 trouvee: ${seriesId}`)

    // 2. Supprimer les images existantes
    if (!skipImages && !dryRun) {
      logger.section('Suppression des images existantes OP05/en')
      const deleteResult = await deleteOnePieceSeriesImages('OP05', 'en')
    ...
    try {
      // 4. Scraper les URLs
      logger.info('\n3. Scraping des URLs de cartes OP05 EN...')
      const cardUrls = await scrapeOP05CardUrls(browser)

      // 5. Parser les URLs
      logger.info('\n4. Parsing des URLs...')
      const cards: OP05Card[] = []

      for (const url of cardUrls) {
        const parsed = parseOP05Url(new URL(url).pathname)
        if (parsed) {
          cards.push(parsed)
        }
      }
    ...
    if (dryRun) {
      ...
      console.log('\nVerifiez: https://www.collectorverse.io/series/onepiece/OP05')
    }
