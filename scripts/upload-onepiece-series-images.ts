/**
 * Upload One Piece Card Game Set Images to Supabase Storage
 *
 * This script uploads the generated One Piece set images to Supabase Storage
 * and updates the series.image_url field in the database.
 *
 * Usage:
 *   npx tsx scripts/upload-onepiece-series-images.ts [--dry-run]
 *
 * Prerequisites:
 *   - Images must be generated in scripts/output/onepiece-images/
...
// Banner dimensions - wide format like Riftbound (ratio ~2.36:1)
...
async function updateSeriesImageUrl(
  ...
  // For One Piece, series are matched by 'code' column
  // Try multiple variations of the code
  const codesToTry = [
    setId,                    // Original: OP01, ST01, PRB01, etc.
    setId.toUpperCase(),      // Uppercase
    setId.toLowerCase(),      // Lowercase
  ]
  ...
  logger.warn(`No series found with code: ${setId}`)
  return false
}
...
async function main() {
  ...
  logger.section('One Piece Card Game Set Image Uploader')
  console.log('')

  // Check input directory exists
  if (!fs.existsSync(CONFIG.inputDir)) {
    logger.error(`Input directory not found: ${CONFIG.inputDir}`)
    logger.info('Run generate-onepiece-images.ts first to generate images')
    process.exit(1)
  }
