/**
 * Script pour re-seeder les cartes promo FR One Piece
 * Source: https://www.opecards.fr/cards/search?page=1&sortBy=number&serie=479&language=FR
 *
 * Ce script:
 * 1. Scrape les 3 pages de promos FR sur opecards.fr
 * 2. Supprime toutes les cartes FR existantes de la série P
 * 3. Insère les nouvelles cartes avec le bon numbering (format P-XXX)
 *
 * Usage:
 *   npx tsx scripts/reseed-onepiece-promo-fr.ts
 *   npx tsx scripts/reseed-onepiece-promo-fr.ts --dry-run
 */
...
const PROMO_SEARCH_URL = `${BASE_URL}/cards/search?sortBy=number&serie=479&language=FR`
 ...
 * Formats supportés:
 * - /cards/p-008-p-dash-pack-volume-2-yamato
 * - /cards/st19-003-sr-pack-promotionnel-vol-0-tashigi
 * - /cards/op09-002-r-fete-decks-debutants-st23-st28-uta
 * - /cards/eb01-027-r-fete-decks-debutants-st23-st28-mr1-das-bones
 */
function parsePromoUrl(url: string): PromoCard | null {
  const slug = url.replace('/cards/', '')

  // Pattern 1: p-{number}-p-{name} (cartes avec code P)
  ...
  // Pattern 2: {prefix}{set}-{number}-{rarity}-{name}
  // Examples: st19-003-sr-..., op09-002-r-..., eb01-027-r-...
...
/**
 * Scrape toutes les URLs de cartes promo FR depuis les 3 pages
 */
async function scrapePromoCardUrls(browser: Browser): Promise<string[]> {
  ...
  try {
    for (let pageNum = 1; pageNum <= 3; pageNum++) {
      ...
      // Extraire les URLs de cartes (tous les formats: p-XXX, stXX-XXX, opXX-XXX, ebXX-XXX)
      const cardUrls = await page.evaluate(() => {
        const links = Array.from(document.querySelectorAll('a[href*="/cards/"]'))
          ...
          .filter(href => {
            ...
            // Accepter tous les formats de cartes promo
            // Pattern: /cards/{code}-{number}-{rarity}-... ou /cards/p-{number}-p-...
      ...
      if (jsonLdScript) {
        try {
          ...
          if (jsonLd.image) {
            ...
            // Prendre la première image FR ou la première disponible
            const frImage = images.find((img: string) => img.includes('/fr/') || img.includes('-fr-'))
            return frImage || images[0]
          }
        } catch (e) {
    ...
    const details = await page.evaluate(() => {
      ...
      if (jsonLdScript) {
        try {
          ...
          if (jsonLd.image) {
            const images = Array.isArray(jsonLd.image) ? jsonLd.image : [jsonLd.image]
            imageUrl = images.find((img: string) => img.includes('/fr/')) || images[0]
          }
        } catch (e) {
          // Ignore
        }
...
/**
 * Supprime toutes les cartes FR de la série P
 */
async function deleteExistingFrCards(seriesId: string): Promise<number> {
  ...
  if (count === 0) {
    logger.info('Aucune carte FR à supprimer')
    return 0
  }

  if (dryRun) {
    logger.info(`[DRY-RUN] ${count} cartes FR seraient supprimées`)
    return count
  }
  ...
  logger.success(`${count} cartes FR supprimées`)
  return count
}
...
async function main() {
  logger.section('Re-seeding One Piece Promo FR')
  ...
  try {
    ...
    try {
      // 3. Scraper les URLs
      logger.info('\n3. Scraping des URLs de cartes promo FR...')
      ...
      logger.section('Suppression des anciennes cartes FR')
      await deleteExistingFrCards(seriesId)
