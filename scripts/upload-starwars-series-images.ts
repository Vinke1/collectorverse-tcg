/**
 * Upload Star Wars Unlimited Set Images to Supabase Storage
 *
 * This script uploads the generated Star Wars set images to Supabase Storage
 * and updates the series.image_url field in the database.
 *
 * Usage:
 *   npx tsx scripts/upload-starwars-series-images.ts [--dry-run]
 *
 * Prerequisites:
 *   - Images must be generated in scripts/output/starwars-images/
...
async function updateSeriesImageUrl(
  ...
  // Try to find the series by code
...
async function main() {
  ...
  logger.section('Star Wars Unlimited Set Image Uploader')
  console.log('')

  // Check input directory exists
  if (!fs.existsSync(CONFIG.inputDir)) {
    logger.error(`Input directory not found: ${CONFIG.inputDir}`)
    logger.info('Run generate-starwars-images.ts first to generate images')
    process.exit(1)
  }
