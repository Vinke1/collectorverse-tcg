/**
 * Script pour re-seeder les cartes OP11 One Piece (EN et FR)
 * Source FR: https://www.opecards.fr/cards/search?page=1&sortBy=releaseR&serie=677&language=FR (6 pages)
 * Source EN: https://www.opecards.fr/cards/search?page=1&sortBy=releaseR&serie=678&language=EN (6 pages)
 *
 * Ce script:
 * 1. Supprime toutes les images OP11 existantes dans Supabase Storage
 * 2. Supprime toutes les cartes existantes de la serie OP11
 * 3. Scrape les pages de OP11 sur opecards.fr (EN et FR)
 ...
 *
 * Usage:
 *   npx tsx scripts/reseed-onepiece-op11.ts
 *   npx tsx scripts/reseed-onepiece-op11.ts --dry-run
 *   npx tsx scripts/reseed-onepiece-op11.ts --lang=en
 *   npx tsx scripts/reseed-onepiece-op11.ts --lang=fr
 *   npx tsx scripts/reseed-onepiece-op11.ts --skip-images
 */
...
// Note: FR n'a pas de prefixe de langue dans les URLs (op11-xxx) contrairement a EN (en-op11-xxx)
const LANG_CONFIG = {
  en: {
    searchUrl: `${BASE_URL}/cards/search?sortBy=releaseR&serie=678&language=EN`,
    totalPages: 6,
    urlPrefix: 'en-op11',
    urlPattern: /^\/cards\/en-op11-(\d{3})-([a-z]+)-(.+)$/i,
    storageLang: 'en'
  },
  fr: {
    searchUrl: `${BASE_URL}/cards/search?sortBy=releaseR&serie=677&language=FR`,
    totalPages: 6,
    urlPrefix: 'op11', // FR n'a pas de prefixe de langue
    urlPattern: /^\/cards\/op11-(\d{3})-([a-z]+)-(.+)$/i,
    storageLang: 'fr'
  }
}
...
interface OP11Card {
  ...
  displayNumber: string // Format: "OP11-001"
  name: string
  rarity: string
...
/**
 * Parse une URL de carte OP11
 * Format EN: /cards/en-op11-{number}-{rarity}-{name}
 * Format FR: /cards/op11-{number}-{rarity}-{name} (pas de prefixe fr-)
 * Exemple: /cards/en-op11-001-l-shanks
 */
function parseOP11Url(url: string, lang: 'en' | 'fr'): OP11Card | null {
...
/**
 * Scrape toutes les URLs de cartes OP11 depuis les pages de recherche
 */
async function scrapeOP11CardUrls(browser: Browser, lang: 'en' | 'fr'): Promise<string[]> {
  const config = LANG_CONFIG[lang]
  ...
  const urlPrefix = config.urlPrefix // 'en-op11' pour EN, 'op11' pour FR
  ...
  try {
    ...
    for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
      ...
      // Extraire les URLs de cartes OP11
      const cardUrls = await page.evaluate((prefix) => {
        const links = Array.from(document.querySelectorAll('a[href*="/cards/"]'))
          ...
          .filter(href => {
            ...
            // Accepter uniquement les cartes OP11 avec le bon prefixe
            // EN: /cards/en-op11-{number}-{rarity}-...
            // FR: /cards/op11-{number}-{rarity}-... (pas de prefixe fr-)
  ...
  logger.success(`Total ${lang.toUpperCase()}: ${uniqueUrls.length} URLs de cartes OP11 uniques`)
 ...
 */
async function fetchCardDetails(page: Page, card: OP11Card): Promise<OP11Card> {
  try {
...
/**
 * Recupere l'ID de la serie OP11 pour One Piece
 */
async function getOP11SeriesId(): Promise<string> {
  ...
  if (seriesError || !series) {
    throw new Error('Serie OP11 non trouvee pour One Piece')
  }
...
/**
 * Supprime toutes les cartes d'une langue de la serie OP11
 */
 ...
 */
async function insertCard(seriesId: string, card: OP11Card): Promise<boolean> {
  if (dryRun) {
...
async function processLanguage(browser: Browser, seriesId: string, lang: 'en' | 'fr'): Promise<{ success: number; error: number }> {
  ...
  if (!skipImages && !dryRun) {
    logger.info(`Suppression des images existantes OP11/${lang}`)
    const deleteResult = await deleteOnePieceSeriesImages('OP11', lang)
  ...
  // 2. Scraper les URLs
  logger.info(`Scraping des URLs de cartes OP11 ${langUpper}...`)
  const cardUrls = await scrapeOP11CardUrls(browser, lang)

  // 3. Parser les URLs
  logger.info('Parsing des URLs...')
  const cards: OP11Card[] = []

  for (const url of cardUrls) {
    const parsed = parseOP11Url(new URL(url).pathname, lang)
    if (parsed) {
      cards.push(parsed)
    }
  }
...
async function main() {
  logger.section('Re-seeding One Piece OP11')
  ...
  try {
    ...
    logger.info('\n1. Recuperation de la serie OP11...')
    const seriesId = await getOP11SeriesId()
    logger.success(`Serie OP11 trouvee: ${seriesId}`)
    ...
    if (dryRun) {
      ...
      console.log('\nVerifiez: https://www.collectorverse.io/series/onepiece/OP11')
    }
