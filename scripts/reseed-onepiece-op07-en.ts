/**
 * Script pour re-seeder les cartes OP07 EN One Piece
 * Source: https://www.opecards.fr/cards/search?page=1&sortBy=releaseR&serie=395&language=EN
 *
 * Ce script:
 * 1. Supprime toutes les images OP07/en existantes dans Supabase Storage
 * 2. Supprime toutes les cartes EN existantes de la serie OP07
 * 3. Scrape les 6 pages de OP07 EN sur opecards.fr
 ...
 *
 * Usage:
 *   npx tsx scripts/reseed-onepiece-op07-en.ts
 *   npx tsx scripts/reseed-onepiece-op07-en.ts --dry-run
 *   npx tsx scripts/reseed-onepiece-op07-en.ts --skip-images
 */
...
const OP07_SEARCH_URL = `${BASE_URL}/cards/search?sortBy=number&serie=395&language=EN`
const TOTAL_PAGES = 6
...
interface OP07Card {
  ...
  displayNumber: string // Format: "OP07-001"
  name: string
  rarity: string
...
function correctName(name: string): string {
  return name
    ...
    // Event cards specifiques OP07
    .replace(/Nothingat All/gi, 'Nothing...At All!!')
    .replace(/Gum Gum King Kong Gatling/gi, 'Gum-Gum King Kong Gatling')
    .replace(/Youre The One Who Should Disappear/gi, "You're The One Who Should Disappear")
    .replace(/I Re Quasar Helllp/gi, "I Require Help!!")
    .replace(/He Possesses The Worlds Most Brilliant Mind/gi, "He Possesses The World's Most Brilliant Mind")
    // Autres corrections
    ...
    .replace(/But I Will Never Doubt A Womans Tears/gi, "But I Will Never Doubt A Woman's Tears")
    // O-Nami
    .replace(/O Nami/gi, 'O-Nami')
    // Mr.
    .replace(/Mr(\d)/gi, 'Mr.$1')
    // Supprimer les prefixes Op07
    .replace(/^Op07\s+/i, '')
...
/**
 * Parse une URL de carte OP07
 * Format: /cards/en-op07-{number}-{rarity}-{name}
 * Exemple: /cards/en-op07-001-l-monkey-d-dragon
 */
function parseOP07Url(url: string): OP07Card | null {
  const slug = url.replace('/cards/', '')

  // Pattern: en-op07-{number}-{rarity}-{name}
  const match = slug.match(/^en-op07-(\d{3})-([a-z]+)-(.+)$/i)
...
/**
 * Scrape toutes les URLs de cartes OP07 EN depuis les 6 pages
 */
async function scrapeOP07CardUrls(browser: Browser): Promise<string[]> {
  ...
  try {
    for (let pageNum = 1; pageNum <= TOTAL_PAGES; pageNum++) {
      const pageUrl = `${OP07_SEARCH_URL}&page=${pageNum}`
      ...
      // Extraire les URLs de cartes OP07 EN
      const cardUrls = await page.evaluate(() => {
        const links = Array.from(document.querySelectorAll('a[href*="/cards/"]'))
          ...
          .filter(href => {
            ...
            // Accepter uniquement les cartes OP07 EN avec prefixe en-
            // Pattern: /cards/en-op07-{number}-{rarity}-...
            return /^\/cards\/en-op07-\d{3}-[a-z]+-/i.test(path)
          })
  ...
  logger.success(`Total: ${uniqueUrls.length} URLs de cartes OP07 uniques`)
 ...
 */
async function fetchCardDetails(page: Page, card: OP07Card): Promise<OP07Card> {
  try {
...
/**
 * Recupere l'ID de la serie OP07 pour One Piece
 */
async function getOP07SeriesId(): Promise<string> {
  ...
  if (seriesError || !series) {
    throw new Error('Serie OP07 non trouvee pour One Piece')
  }
...
/**
 * Supprime toutes les cartes EN de la serie OP07
 */
 ...
 */
async function insertCard(seriesId: string, card: OP07Card): Promise<boolean> {
  if (dryRun) {
...
async function main() {
  logger.section('Re-seeding One Piece OP07 EN')
  console.log(`Source: ${OP07_SEARCH_URL}`)
  console.log(`Mode: ${dryRun ? 'DRY-RUN' : 'PRODUCTION'}`)
  console.log(`Skip images: ${skipImages}`)

  try {
    // 1. Recuperer l'ID de la serie
    logger.info('\n1. Recuperation de la serie OP07...')
    const seriesId = await getOP07SeriesId()
    logger.success(`Serie OP07 trouvee: ${seriesId}`)

    // 2. Supprimer les images existantes
    if (!skipImages && !dryRun) {
      logger.section('Suppression des images existantes OP07/en')
      const deleteResult = await deleteOnePieceSeriesImages('OP07', 'en')
    ...
    try {
      // 4. Scraper les URLs
      logger.info('\n3. Scraping des URLs de cartes OP07 EN...')
      const cardUrls = await scrapeOP07CardUrls(browser)

      // 5. Parser les URLs
      logger.info('\n4. Parsing des URLs...')
      const cards: OP07Card[] = []

      for (const url of cardUrls) {
        const parsed = parseOP07Url(new URL(url).pathname)
        if (parsed) {
          cards.push(parsed)
        }
      }
    ...
    if (dryRun) {
      ...
      console.log('\nVerifiez: https://www.collectorverse.io/series/onepiece/OP07')
    }
