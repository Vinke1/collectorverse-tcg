/**
 * Script pour re-seeder les cartes OP08 EN One Piece
 * Source: https://www.opecards.fr/cards/search?page=1&sortBy=number&serie=414&language=EN
 *
 * Ce script:
 * 1. Supprime toutes les images OP08/en existantes dans Supabase Storage
 * 2. Supprime toutes les cartes EN existantes de la serie OP08
 * 3. Scrape les 6 pages de OP08 EN sur opecards.fr
 ...
 *
 * Usage:
 *   npx tsx scripts/reseed-onepiece-op08-en.ts
 *   npx tsx scripts/reseed-onepiece-op08-en.ts --dry-run
 *   npx tsx scripts/reseed-onepiece-op08-en.ts --skip-images
 */
...
const OP08_SEARCH_URL = `${BASE_URL}/cards/search?sortBy=number&serie=414&language=EN`
const TOTAL_PAGES = 6
...
interface OP08Card {
  ...
  displayNumber: string // Format: "OP08-001"
  name: string
  rarity: string
...
/**
 * Parse une URL de carte OP08
 * Format: /cards/en-op08-{number}-{rarity}-{name}
 * Exemple: /cards/en-op08-001-l-monkey-d-dragon
 */
function parseOP08Url(url: string): OP08Card | null {
  const slug = url.replace('/cards/', '')

  // Pattern: en-op08-{number}-{rarity}-{name}
  const match = slug.match(/^en-op08-(\d{3})-([a-z]+)-(.+)$/i)
...
/**
 * Scrape toutes les URLs de cartes OP08 EN depuis les 6 pages
 */
async function scrapeOP08CardUrls(browser: Browser): Promise<string[]> {
  ...
  try {
    for (let pageNum = 1; pageNum <= TOTAL_PAGES; pageNum++) {
      const pageUrl = `${OP08_SEARCH_URL}&page=${pageNum}`
      ...
      // Extraire les URLs de cartes OP08 EN
      const cardUrls = await page.evaluate(() => {
        const links = Array.from(document.querySelectorAll('a[href*="/cards/"]'))
          ...
          .filter(href => {
            ...
            // Accepter uniquement les cartes OP08 EN avec prefixe en-
            // Pattern: /cards/en-op08-{number}-{rarity}-...
            return /^\/cards\/en-op08-\d{3}-[a-z]+-/i.test(path)
          })
  ...
  logger.success(`Total: ${uniqueUrls.length} URLs de cartes OP08 uniques`)
 ...
 */
async function fetchCardDetails(page: Page, card: OP08Card): Promise<OP08Card> {
  try {
...
/**
 * Recupere l'ID de la serie OP08 pour One Piece
 */
async function getOP08SeriesId(): Promise<string> {
  ...
  if (seriesError || !series) {
    throw new Error('Serie OP08 non trouvee pour One Piece')
  }
...
/**
 * Supprime toutes les cartes EN de la serie OP08
 */
 ...
 */
async function insertCard(seriesId: string, card: OP08Card): Promise<boolean> {
  if (dryRun) {
...
async function main() {
  logger.section('Re-seeding One Piece OP08 EN')
  console.log(`Source: ${OP08_SEARCH_URL}`)
  console.log(`Mode: ${dryRun ? 'DRY-RUN' : 'PRODUCTION'}`)
  console.log(`Skip images: ${skipImages}`)

  try {
    // 1. Recuperer l'ID de la serie
    logger.info('\n1. Recuperation de la serie OP08...')
    const seriesId = await getOP08SeriesId()
    logger.success(`Serie OP08 trouvee: ${seriesId}`)

    // 2. Supprimer les images existantes
    if (!skipImages && !dryRun) {
      logger.section('Suppression des images existantes OP08/en')
      const deleteResult = await deleteOnePieceSeriesImages('OP08', 'en')
    ...
    try {
      // 4. Scraper les URLs
      logger.info('\n3. Scraping des URLs de cartes OP08 EN...')
      const cardUrls = await scrapeOP08CardUrls(browser)

      // 5. Parser les URLs
      logger.info('\n4. Parsing des URLs...')
      const cards: OP08Card[] = []

      for (const url of cardUrls) {
        const parsed = parseOP08Url(new URL(url).pathname)
        if (parsed) {
          cards.push(parsed)
        }
      }
    ...
    if (dryRun) {
      ...
      console.log('\nVerifiez: https://www.collectorverse.io/series/onepiece/OP08')
    }
