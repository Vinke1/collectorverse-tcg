/**
 * One Piece Card Game Set Image Generator using Higgsfield Cloud API
 *
 * This script generates banner images for One Piece TCG sets using the Higgsfield AI API.
 *
 * Usage:
 *   npx tsx scripts/generate-onepiece-images.ts [--start <index>] [--count <number>] [--dry-run]
 *
...
import { ONEPIECE_SET_PROMPTS, TOTAL_PROMPTS } from './data/onepiece-set-prompts'
...
const CONFIG = {
  ...
  outputDir: 'scripts/output/onepiece-images',
  // Progress file to track completed generations
  progressFile: 'scripts/output/onepiece-images-progress.json'
}
...
async function main() {
  ...
  console.log('\n\x1b[36m╔══════════════════════════════════════════════════════════╗\x1b[0m')
  console.log('\x1b[36m║   One Piece Card Game Image Generator - Higgsfield AI    ║\x1b[0m')
  ...
  const setsToProcess = ONEPIECE_SET_PROMPTS.slice(startIndex, startIndex + countArg).filter(
